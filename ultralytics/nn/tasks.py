...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        # 处理新模块 - 修改的关键部分
        if m in {PseudoAIFI, CSPDarknetBlock, CSPDarknetStage, DynamicPseudoCCFM, WeakSupervisionWrapper}:
            # 对于新模块，需要特殊处理参数
            if m is PseudoAIFI:
                # PseudoAIFI(dim, num_heads=8, dropout=0.1)
                # dim应该等于输入通道数
                c1 = ch[f]
                if args:
                    # 第一个参数是dim，如果未提供则使用c1
                    if len(args) >= 1:
                        dim = args[0] if isinstance(args[0], (int, float)) else c1
                        if len(args) >= 2:
                            num_heads = args[1]
                            if len(args) >= 3:
                                dropout = args[2]
                                m_ = m(dim, num_heads, dropout)
                            else:
                                m_ = m(dim, num_heads)
                        else:
                            m_ = m(dim)
                    else:
                        m_ = m(c1)  # 使用输入通道数作为dim
                else:
                    m_ = m(c1)
                c2 = c1  # 输出通道数与输入相同

            elif m is CSPDarknetBlock:
                # CSPDarknetBlock(c1, c2, n=1, shortcut=True, g=1, e=0.5)
                c1 = ch[f]
                if args and len(args) >= 1:
                    c2_arg = args[0] if isinstance(args[0], (int, float)) else 128
                    c2 = make_divisible(min(c2_arg, max_channels) * width, 8)
                    ...
                    if len(args) >= 2:
                        ...
                        if len(args) >= 3:
                            ...
                            if len(args) >= 4:
                                ...
                                if len(args) >= 5:
                                    e = args[4]
                                    m_ = m(c1, c2, n_blocks, shortcut, g, e)
                                else:
                                    m_ = m(c1, c2, n_blocks, shortcut, g)
                            else:
                                m_ = m(c1, c2, n_blocks, shortcut)
                        else:
                            m_ = m(c1, c2, n_blocks)
                    else:
                        m_ = m(c1, c2)
                else:
                    c2 = make_divisible(min(128, max_channels) * width, 8)
                    m_ = m(c1, c2)

            elif m is CSPDarknetStage:
                # CSPDarknetStage(c1, c2, n=1, stride=2)
                c1 = ch[f]
                if args and len(args) >= 1:
                    c2_arg = args[0] if isinstance(args[0], (int, float)) else 256
                    c2 = make_divisible(min(c2_arg, max_channels) * width, 8)
                    ...
                    if len(args) >= 2:
                        ...
                        if len(args) >= 3:
                            stride = args[2]
                            m_ = m(c1, c2, n_blocks, stride)
                        else:
                            m_ = m(c1, c2, n_blocks)
                    else:
                        m_ = m(c1, c2)
                else:
                    c2 = make_divisible(min(256, max_channels) * width, 8)
                    m_ = m(c1, c2)

            elif m is DynamicPseudoCCFM:
                # DynamicPseudoCCFM(in_channels_list, out_channels, use_cache=True, cache_size=100)
                # 这里的f应该是多个输入层的索引列表
                ...
                if args and len(args) >= 1:
                    # args[0]应该是输出通道数
                    out_channels = args[0] if isinstance(args[0], (int, float)) else 256
                    out_channels = make_divisible(min(out_channels, max_channels) * width, 8)

                    if len(args) >= 2:
                        use_cache = args[1] if isinstance(args[1], bool) else True
                        if len(args) >= 3:
                            cache_size = args[2] if isinstance(args[2], (int, float)) else 100
                            m_ = m(in_channels, out_channels, use_cache, cache_size)
                        else:
                            m_ = m(in_channels, out_channels, use_cache)
                    else:
                        m_ = m(in_channels, out_channels)
                else:
                    out_channels = make_divisible(min(256, max_channels) * width, 8)
                    m_ = m(in_channels, out_channels)

                c2 = out_channels

            elif m is WeakSupervisionWrapper:
                # WeakSupervisionWrapper(module, module_type='aifi')
                # 这里args[0]应该是一个模块配置，我们需要递归解析
                c1 = ch[f]
                if args and len(args) >= 1:
                    module_config = args[0]
                    module_type = args[1] if len(args) >= 2 else 'aifi'

                    # 递归解析内部模块
                    if isinstance(module_config, list) and len(module_config) >= 1:
                        inner_module_name = module_config[0]
                        inner_module_args = module_config[1:] if len(module_config) > 1 else []

                        # 获取内部模块类
                        inner_module_class = globals().get(inner_module_name)
                        if inner_module_class:
                            # 构造内部模块
                            if inner_module_class is PseudoAIFI:
                                inner_module = PseudoAIFI(c1, *inner_module_args) if inner_module_args else PseudoAIFI(
                                    c1)
                            else:
                                # 其他模块的处理
                                inner_module = inner_module_class(*inner_module_args)

                            m_ = m(inner_module, module_type)
                        else:
                            LOGGER.warning(f"Unknown inner module: {inner_module_name}")
                            m_ = m(torch.nn.Identity(), module_type)
                    else:
                        m_ = m(torch.nn.Identity(), module_type)
                else:
                    m_ = m(torch.nn.Identity(), 'aifi')
